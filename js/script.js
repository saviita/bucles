...
const getTenNumbers = (tenNumbers) => {
  ...
  for (const number of tenNumbers) {
    resultSum = resultSum + tenNumbers[counter];
  }
...
const twoArrays = () => {
  ...
  for (counter = 0; counter <= 5; counter++) {
    if (secondArray.includes(firstArray[counter])) {
      repeatNumber.push(firstArray[counter]);
    }
  }
...
const tenSquared = (tenNumbers) => {
  for (const squaredCubed of tenNumbers) {
    ...
    console.log(
      `Número: ${squaredCubed} - Cuadrado: ${numSquared} - Cubo: ${numCubed}`
    );
  }
};
...
const getWord = (word) => {
  ...
  for (counter = 0; counter < word.length; counter++) {
    if (
      word[counter].includes("a") ||
      word[counter].includes("e") ||
      word[counter].includes("i") ||
      word[counter].includes("o") ||
      word[counter].includes("u")
    ) {
      word.charAt(counter).toUpperCase();
    }
  }
  console.log(word);
};
...
const getPhrase = (phrase) => {
  ...
  for (counter = 0; counter < phrase.length; counter++) {
    if (phrase[counter].includes("a") || phrase[counter].includes("A")) {
      resultA = resultA + 1;
    } else if (phrase[counter].includes("e") || phrase[counter].includes("E")) {
      resultE = resultE + 1;
    } else if (phrase[counter].includes("i") || phrase[counter].includes("I")) {
      resultI = resultI + 1;
    } else if (phrase[counter].includes("o") || phrase[counter].includes("O")) {
      resultO = resultO + 1;
    } else if (phrase[counter].includes("u") || phrase[counter].includes("U")) {
      resultU = resultU + 1;
    }
  }

  console.log(
    `a:${resultA}, e:${resultE}, i:${resultI}, o:${resultO}, u:${resultU}`
  );
};
...
const getTwoWords = (word1, word2) => {
  let insertWord = "";
  const longestWord = Math.min(word1.length, word2.length);

  for (counter = 0; counter < longestWord; counter++) {
    insertWord = insertWord + word1.charAt(counter) + word2.charAt(counter);
  }
...
const getReverseWord = (word) => {
  ...
  for (counter = word.length - 1; counter >= 0; counter--) {
...
const getTenArray = (tenArray) => {
  ...
  for (counter = 0; counter < 10; counter++) {
    const resultMultiply = tenArray[counter] * randomNumber;
...
const getFiveWords = (fiveWords) => {
  ...
  for (const firstLast of fiveWords) {
    firstLastLetter.push(
      firstLast.charAt(0).toUpperCase(),
      firstLast.charAt(firstLast.length - 1).toUpperCase()
    );
  }
